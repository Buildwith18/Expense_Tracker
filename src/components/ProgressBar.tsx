...
const ProgressBar: React.FC<ProgressBarProps> = ({
  ...
  const getColorClass = () => {
    if (safePercentage >= 90) return 'bg-red-500';
    if (safePercentage >= 75) return 'bg-yellow-500';
    return color;
  };
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700 transition-colors duration-200">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-sm font-medium text-gray-600 dark:text-gray-400">{safePercentage.toFixed(1)}% used</span>
      </div>
      ...
      <div className="mt-2 text-center">
        <span className="text-sm text-gray-600 dark:text-gray-400">
          ₹{safeSpent.toFixed(2)} spent of ₹{safeBudget.toFixed(2)} budget
        </span>
      </div>
    </div>
  );
